...
class AnalisadorDePublicacoes:
    ...
    def __init__(self, grupo):
        ...
        self.parserFile = xml.dom.minidom.parse(
            './scriptLattes/internacionalizacao/parserFileConfig.xml')  # read the file just the fist time
        self.paises = {
    ...
    def procurarPais(self, dataDoi, nomeDePais, urlDOI):
        ...
        nomeDePais = HTMLParser().unescape(nomeDePais)
        nomeDePais = unicodedata.normalize('NFKD', nomeDePais)#.encode('ascii', 'ignore')
        if len(nomeDePais) <= 0:
            return False
        if len(dataDoi) == 2:
            doihtml = dataDoi[0]
        ...
        # tentamos 3 vezes baixar a página web associado ao DOI
            ...
            while tentativa <= 1:
                try:
                    ...
                    rawDOIhtml = HTMLParser().unescape(rawDOIhtml)
                    ...
                    print (f'[AVISO] Tentativa ' + str(tentativa) + ': DOI não está disponível na internet: {urlDOI}')
                    time.sleep(10)
                    rawDOIhtml = None
                    tentativa += 1
                    continue
